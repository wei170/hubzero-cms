...
Toolbar::title(Lang::txt('COM_DRWHO') . ': ' . Lang::txt('COM_DRWHO_CHARACTERS'));
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th scope="col" class="priority-5"><?php echo Html::grid('sort', 'COM_DRWHO_COL_ID', 'id', @$this->filters['sort_Dir'], @$this->filters['sort']); ?></th>
				<th scope="col"><?php echo Html::grid('sort', 'COM_DRWHO_COL_NAME', 'name', @$this->filters['sort_Dir'], @$this->filters['sort']); ?></th>
				<th scope="col" class="priority-3"><?php echo Html::grid('sort', 'COM_DRWHO_COL_STATE', 'state', @$this->filters['sort_Dir'], @$this->filters['sort']); ?></th>
				<th scope="col" class="priority-4"><?php echo Html::grid('sort', 'COM_DRWHO_COL_THE_DOCTOR', 'doctor', @$this->filters['sort_Dir'], @$this->filters['sort']); ?></th>
				<th scope="col" class="priority-4"><?php echo Html::grid('sort', 'COM_DRWHO_COL_FRIEND', 'friend', @$this->filters['sort_Dir'], @$this->filters['sort']); ?></th>
				<th scope="col" class="priority-4"><?php echo Html::grid('sort', 'COM_DRWHO_COL_ENEMY', 'enemy', @$this->filters['sort_Dir'], @$this->filters['sort']); ?></th>
			</tr>
		</thead>
		<tfoot>
			<tr>
		...
		foreach ($this->rows as $row) : ?>
			<tr class="<?php echo "row$k"; ?>">
				...
				<td class="priority-4">
					<?php echo $row->isDoctor() ? '<span class="state yes"><span>' . Lang::txt('JYES') . '</span></span>' : '<span class="state no"><span>' . Lang::txt('JNO') . '</span></span>'; ?>
				</td>
				<td class="priority-4">
					<?php echo $row->isFriend() ? '<span class="state yes"><span>' . Lang::txt('JYES') . '</span></span>' : '<span class="state no"><span>' . Lang::txt('JNO') . '</span></span>'; ?>
				</td>
				<td class="priority-4">
					<?php echo $row->isEnemy() ? '<span class="state yes"><span>' . Lang::txt('JYES') . '</span></span>' : '<span class="state no"><span>' . Lang::txt('JNO') . '</span></span>'; ?>
				</td>
			</tr>
			<?php
			$i++;
