 ...
 *                     /two.php
 */
class Characters extends AdminController
{
	/**
	 * Execute a task
	 *
	...
	public function editTask($row=null)
	{
		...
		if (!is_object($row))
		{
			...
			// Load the article
			$row = Character::oneOrNew($id);
		}

		$this->view->row = $row;
		$this->view->seasons = Season::all()->ordered();
	...
	public function saveTask()
	{
		...
		$row = Character::oneOrNew($fields['id'])->set($fields);
	...
	public function removeTask()
	{
		...
		if (count($ids) > 0)
		{
			...
			foreach ($ids as $id)
			{
				...
				$entry = Character::oneOrFail(intval($id));
	...
	public function stateTask()
	{
		...
		foreach ($ids as $id)
		{
			...
			$row = Character::oneOrNew(intval($id))->set(array('state' => $state));
