<?php
namespace Components\Drwho\Admin\Controllers;
 ...
 *                     /two.php
 */
class Seasons extends AdminController
{
	/**
	 * Execute a task
	 *
	...
	public function editTask($row=null)
	{
		...
		if (!is_object($row))
		{
			...
			// Load the record
			$row = Season::oneOrNew($id);
		}

		$this->view->row = $row;

		// Get a list of the doctors
		$this->view->doctors = Character::all()
			->whereEquals('state', 1)
			->whereEquals('doctor', 1);
	...
	public function saveTask()
	{
		...
		$row = Season::oneOrNew($fields['id'])->set($fields);
	...
	public function removeTask()
	{
		...
		if (count($ids) > 0)
		{
			...
			foreach ($ids as $id)
			{
				$entry = Season::oneOrFail(intval($id));
		...
		App::redirect(
			Route::url('index.php?option=' . $this->_option . '&controller=' . $this->_controller, false),
			($removed ? Lang::txt('COM_DRWHO_ENTRIES_DELETED') : null)
		);
	}
	...
	public function stateTask()
	{
		...
		if (count($ids) < 1)
		{
			...
			App::redirect(
				Route::url('index.php?option=' . $this->_option . '&controller=' . $this->_controller, false),
				Lang::txt('COM_DRWHO_SELECT_ENTRY_TO', $this->_task),
				'error'
			);
			return;
		}
		...
		foreach ($ids as $id)
		{
			...
			$row = Season::oneOrNew(intval($id))->set(array('state' => $state));
